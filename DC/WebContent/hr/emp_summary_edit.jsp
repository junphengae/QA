	...
	<div class="wrap_body">
		<div class="body_content">
			<div class="content_head">
				...
				<div class="right">
					<button class="btn_box" onclick="javascript: window.location='emp_summary.jsp?per_id=<%=per_id%>&month=<%=month%>&year=<%=year%>';">ย้อนกลับ</button>
				</div>
				<div class="clear"></div>
			</div>
				<%
			...
			<div class="content_body">
							...
							submitHandler: function(){
								...
								$.post('../EmpManageServlet',addData,function(resData){
									...
									if (resData.status == "success") {
										...
										window.location = 'emp_summary.jsp?per_id=<%=per_id%>&month=<%=month%>&year=<%=year%>';
									} else {	
				...
				<input type="hidden" name="per_id" value="<%=per_id%>">
				<input type="hidden" name="salary_type" value="<%=entity.getSalary_type()%>">
				<input type="hidden" name="month" value="<%=month%>">
				<input type="hidden" name="year" value="<%=year%>">
				<input type="hidden" id="salary" name="salary" value="<%=entity.getSalary()%>">
				<input type="hidden" name="update_by" value="<%=securProfile.getPersonal().getPer_id()%>">
						...
						<table cellpadding="3" cellspacing="3" border="0" style="margin: 0 auto;" width="95%">
							<tbody>
								...
								<tr>
									...
									<td>: <%=coco[0] %> <% if (coco[0].length()==0){}else{ %>&nbsp;ลูก<%}%>  
								...
								<tr>
									...
									<td>: <%=Money.money(coco[1]) %> <% if (coco[1].length()==0){}else{ %>&nbsp;บาท<%}%>
								...
								<tr>
									...
									<td>: <%=coco[2] %> <% if (coco[2].length()==0){}else{ %>&nbsp;ลูก<%}%> 
											<input type="hidden" name="coco_ot_sum" value="<%=coco[2] %>">
									</td> 
								</tr>
								<tr>
									<td><label>ค่าแรงมะพร้าวโอที</label></td>
									<td>: <%=Money.money(coco[3]) %> <% if (coco[3].length()==0){}else{ %>&nbsp;บาท<%}%>
								...
								<tr>
									...
									<td>: <input type="text" name="bonus" id="bonus" class="txt_box" value="<%=entity.getBonus() %>" autocomplete="off" > บาท</td>
								</tr>
								<tr>
									<td><label>เบี้ยขยัน </label></td>
									<td>: <input type="text" name="diligence" id="diligence" class="txt_box" value="<%=entity.getDiligence() %>" autocomplete="off" > บาท</td>
								</tr>
								<tr>
									<td><label>เบี้ยเลี้ยง</label></td>
									<td>: <input type="text" name="allowance" id="allowance" class="txt_box" value="<%=entity.getAllowance() %>" autcomplete="off"> บาท</td>
								</tr>
								<tr>
									<td><label>เงินเดือนส่วนต่าง</label></td>
									<td>: <input type="text" name="bal_salary" id="bal_salary" class="txt_box" value="<%=entity.getBal_salary() %>" autcomplete="off"> บาท</td>
								</tr>
								<tr>
					...
					<fieldset class="fset">
						...
						<table cellpadding="3" cellspacing="3" border="0" style="margin: 0 auto;" width="95%">
							<tbody>
								...
								<tr>
									<td><label>ภาษี </label> <input type="text" name="tax_rate" id="tax_rate" class="txt_box s50" value="<%=entity.getTax_rate() %>" autocomplete="off" > %</td>
									<td>: 
										<input type="text" name="tax" id="tax" class="txt_box" value="<%=entity.getTax()%>"> บาท
										<span id="tax_show" ></span> 
								
									</td>
								</tr>
								<tr>
									<td><label>ประกันสังคม </label> </td>
									<td>: <input type="text" name="ss" id="ss" class="txt_box" value="<%=entity.getSs() %>" autocomplete="off" > บาท</td>
								</tr>
								<tr>
									<td><label>เงินสะสม</label></td>
									<td>: <input type="text" id="savings" name="savings" class="txt_box" value="<%=entity.getSavings() %>" autocomplete="off" > บาท</td>
								</tr>
								<tr>
									<td><label>เบิกล่วงหน้า</label></td>
									<td>: <input type="text" id="pay_advance" name="pay_advance" class="txt_box" value="<%=entity.getPay_advance() %>" autocomplete="off" > บาท</td>
								</tr>
								<tr>
									<td><label>ขาดงาน</label></td>
									<td>: <input type="text" id="missing" name="missing" class="txt_box" value="<%=entity.getMissing() %>" autocomplete="off" > บาท </td>
								</tr>
								<tr>
									<td><label>เงินหักอื่นๆ </label></td>
									<td>: <input type="text" id="deduction" name="deduction" class="txt_box" value="<%=entity.getDeduction() %>" autocomplete="off" > บาท
									<label>หมายเหตุ </label>
									<input type="text" id="remark" name="remark" class="txt_box s250" value="<%=entity.getRemark() %>"  autocomplete="off" >
									</td>
								</tr>
										...
										<tr>
											...
											<td> 
												<div title="รายละเอียด" class="thickbox pointer" lang="leave_view.jsp?per_id=<%=per_id%>&leave_type_id=2&leave_type_id1=8&month=<%=month%>&year=<%=year%>&width=800&height=650">: 
													...
													<%-- <%=leave_b%>  <%if (leave_b.isEmpty()){}else{ %>&nbsp;วัน <%} %> --%>
										...
										<tr>
											...
											<td>
												<div title="รายละเอียด" class="thickbox pointer" lang="leave_view.jsp?per_id=<%=per_id%>&leave_type_id=1&leave_type_id1=7&month=<%=month%>&year=<%=year%>&width=800&height=650">: 
													...
													<%-- <%=leave_s %> <% if (leave_s.isEmpty()){}else{ %>&nbsp;วัน <%}%>  --%>
										...
										<tr>
											...
											<td> 
												<div title="รายละเอียด" class="thickbox pointer" lang="leave_view.jsp?per_id=<%=per_id%>&leave_type_id=3&leave_type_id1=9&month=<%=month%>&year=<%=year%>&width=800&height=650">: 
													...
													<%-- <%=leave_v %> <% if (leave_v.isEmpty()){}else{ %>&nbsp;วัน <%}%> --%>
								...
								<table cellpadding="3" cellspacing="3" border="0" style="margin: 0 auto;" width="90%">
									<tbody>
										...
										<%
												...
												Double limit_b = Double.parseDouble(Salary.select(per_id).getLimit_business());
												//Double leav_b = Double.parseDouble(leave_b_y);
												Double lb_total = limit_b - sum_b ;
												String slb_total = Double.toString(lb_total); 
												////System.out.println(leave_b_y);
										%> 
										<tr>
											<td><label>ลากิจคงเหลือ/ปี</label></td>
											<td>: <%=slb_total %> <% if (slb_total.length()==0){}else{ %>&nbsp;วัน <%}%> 
											<input type="hidden" name="bal_business" value="<%=slb_total%>">
											</td>
										</tr>
										<%
												//String leave_s_y = Leave.count_leave_year(per_id, "1", year,month);
												Double limit_s = Double.parseDouble(Salary.select(per_id).getLimit_sick());
												//int leav_s = Integer.parseInt(leave_s_y);
												Double ls_total = limit_s - sum_s;
												String sls_total = Double.toString(ls_total);
										%> 
										<tr>
											<td><label>ลาป่วยคงเหลือ/ปี</label></td>
											<td>: <%=sls_total %> <% if (sls_total.length()==0){}else{ %>&nbsp;วัน <%}%> 
											<input type="hidden" name="bal_sick" value="<%=sls_total%>">
											</td>
										</tr>
										<%
												//String leave_v_y = Leave.count_leave_year(per_id, "3", year,month);
												Double limit_v = Double.parseDouble(Salary.select(per_id).getLimit_vacation());
												//int leav_v = Integer.parseInt(leave_v_y);
												Double lv_total = limit_v - sum_v;
												String slv_total = Double.toString(lv_total);
										%> 
										<tr>
											<td><label>ลาพักร้อนคงเหลือ/ปี</label></td>
											<td>: <%=slv_total %> <% if (slv_total.length()==0){}else{ %>&nbsp;วัน <%}%> 
						...
						<%-- <table cellpadding="3" cellspacing="3" border="0" style="margin: 0 auto;" width="95%">
							<tbody>
								...
								<tr>
									...
									<td>: <%=entity.getLeave_business()%>  <%if (entity.getLeave_business().isEmpty()){}else{ %>&nbsp;วัน <%} %>
									<input type="hidden" name="leave_business" value="<%=entity.getLeave_business()%>">
									</td>
								...
								<tr>
									...
									<input type="hidden" name="leave_sick" value="<%=entity.getLeave_sick()%>">
									</td>
								</tr>
								<tr>
									<td><label>ลาพักร้อน </label></td>
									<td>: <%=entity.getLeave_vacation() %> <% if (entity.getLeave_vacation().isEmpty()){}else{ %>&nbsp;วัน <%}%>
									<input type="hidden" name="leave_vacation" value="<%=entity.getLeave_vacation()%>">
									</td>
								</tr>
					...
					<div class="center txt_center m_top10">
						<input type="submit" id="btnAdd" value="บันทึกการแก้ไข" class="btn_box btn_confirm">
						<input type="hidden" name="action" value="edit_emp_sum">
					</div>
				</form>
