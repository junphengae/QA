						...
						<legend>ข้อมูลการเบิก</legend>					
							<table width="100%">
								<tbody>
									...
									<% if(invMaster.getGroup_id().equalsIgnoreCase("3")){ %>
									<tr>
										<td>หมายเลขโทรศัพท์</td>
										<td>: <input type="text" autocomplete="off" name="phone" id="phone" class="txt_box s150"></td>
									</tr>
									<%}else if(invMaster.getGroup_id().equalsIgnoreCase("4")){ %>
									<tr>
										<td>เลขที่กรรมธันย์</td>
										<td>: <input type="text" autocomplete="off" name="ins" id="ins" class="txt_box s150"></td>
									</tr>
									<%}else{
										if(invMaster.getSerial().equalsIgnoreCase("n")){%>
										<%}else{ %>
											<tr id="tr_lot_no">
									...
									<tr>
										<td colspan="2" align="center">
											<input type="hidden" name="order_item_id" id="order_item_id" value="<%=WebUtils.getReqString(request,"order_item_id")%>">
											<input type="hidden" name="request_type" id="request_type" value="IV">		
											...
											<input type="hidden" name="action" value="withdraw_for_sale"> 
											<input type="hidden" name="mat_code" value="<%=mat_code%>">
											<input type="hidden" name="group_id" value="<%=invMaster.getGroup_id()%>">
											<input type="hidden" name="request_no" value="<%=order_id%>"> 
								...
								$('#btn_withdraw').click(function(){
									...
									$.post('../OutletManagement',data,function(resData){
										...
										if (resData.status == 'success') {
											...
											if(resData.link == 'new'){
												window.location = 'inv_withdraw.jsp';
											}else{
												window.location = 'sale_order_view.jsp?order_id=' + resData.order_id;
											}
