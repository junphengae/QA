...
<form id="create_order_form" onsubmit="return false;">
	<fieldset class="fset s800 center m_top10">
		...
		<table class="bg-image s_auto" style="color: #fff;">
			...
			<tr id="vendor_<%=vendor.getVendor_id()%>">
				<td><input type="radio" name="vendor_id" id="vendor_id" value="<%=vendor.getVendor_id()%>" ></td>
				<td><%=vendor.getVendor_name()%></td>
				<td align="center"><%=masterVendor.getVendor_moq()%>
				<%//=vendor.getVendor_id()%>
			...
			<tbody>
				<tr align="center" valign="bottom" height="25">
					<td colspan="2">
						...
						<input type="reset" name="reset" class="btn_box" title="ปิดหน้าจอ" value="Close" onclick="window.location='purchase_manage.jsp';">
	...
	<fieldset class="fset s800 center m_top10">
		<legend>Summary of the past 3 months: <%=entity.getDescription()%></legend>
			...
			<tbody>
				<tr>
					<td><%=WebUtils.getDateValue(pr.getCreate_date())%></td>
					<td><%=pr.getPo() %></td>
					<td align="right"><%=Money.moneyInteger(pr.getOrder_qty())%></td>
