...
Iterator ite = DcpurchaseOrder.selectWithCTRL(ctrl, paramList).iterator();
%>
			...
			<div class="content_body">
				<div class="txt_center">
					<form style="margin: 0; padding: 0;" action="po_manage.jsp" id="search" method="get">
						สถานะ: 
						<bmp:ComboBox name="status" styleClass="txt_box s100" listData="<%=DcpurchaseRequest.statusDropdown4PO()%>" value="<%=status%>">
							<bmp:option value="" text="แสดงทั้งหมด"></bmp:option>
						</bmp:ComboBox>
						&nbsp;
						
						ตัวแทนจำหน่าย:
						<bmp:ComboBox name="vendor_id" styleClass="txt_box s120" listData="<%=DcpurchaseRequest.vendorDropdown()%>" value="<%=vendor_id%>">
							<bmp:option value="" text="แสดงทั้งหมด"></bmp:option>
						</bmp:ComboBox>
						&nbsp;
				...
				<table class="bg-image s_auto">
					...
					<tbody>
						<%
							boolean has = true;
							while(ite.hasNext()) {
								DcpurchaseOrder entity = (DcpurchaseOrder) ite.next();
								has = false;
						%>
							<tr>
								<td><%=entity.getPo()%></td>
								...
								<td align="center"><%=DcpurchaseRequest.status(entity.getStatus())%></td>
								<td align="center">
