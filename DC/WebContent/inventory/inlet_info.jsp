	...
	<div class="wrap_body">
		<div class="body_content">
			<div class="content_head">
				<div class="left">
					1: รับเข้าสินค้า | 2: รับเข้าจากการสั่งซื้อ | 3: แสดงข้อมูลคงคลัง
				</div>
				<div class="right">
			...
			<div class="content_body">
				...
				<table width="100%">
					<tbody>
						...
						<tr>
							<td>จำนวนทั้งหมด</td>
							<td>: <%=master.getBalance()+ " " + master.getDes_unit()%></td>
						</tr>
					</tbody>
				</table>
					...
					<div class="s400 left">
						<table width="100%">
							<tbody>
								...
								%>
								<tr>
									<td width="40%">หมายเลข Serial (Serial No.)</td>
									<td width="60%">: <%=LOT.getSerial()%></td>
								</tr>
								<%}else{ %>
								<tr>
				...
				<table class="bg-image s900">
					<thead>
						<tr>
							...
							if(master.getSerial().equalsIgnoreCase("y")){
							%>
							<th valign="top" align="center" width="20%">Serial เลขที่ </th>
							<%}else{ %>
							<th valign="top" align="center" width="20%">Lot เลขที่ </th>
							<%} %>
							<th valign="top" align="center" width="15%">เลขที่ใบสั่งซื้อ</th>
					...
					%>
						<tr>
							<td><%=(master.getSerial().equalsIgnoreCase("y")?lot.getSerial():lot.getLot_no())%></td>
							<td><%=lot.getPo()%></td>
							<td><%=lot.getInvoice()%></td>
				...
				<input type="hidden" id="total" value="<%=Money.moneyNoCommas(total)%>">
				
				<div class="center txt_center m_top10">
					<button class="btn_box" onclick="window.location='inlet_invoice.jsp?po=<%=WebUtils.getReqString(request, "po")%>';">ย้อนกลับไปใบสั่งซื้อเลขที่ <%=WebUtils.getReqString(request, "po")%></button>
					<button class="btn_box" onclick="window.location='inlet_invoice.jsp';">รับเข้าใหม่</button>
				</div>
			</div>
		</div>
	</div>
