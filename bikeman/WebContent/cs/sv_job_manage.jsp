...
function Restatus(job_id){
	
			 
			if (confirm('ยืนยันการย้อนกลับสถานเป็น MA Request')) { 
				ajax_load();
				$.post('../PartSaleManage','action=restatus&update_by=<%=securProfile.getPersonal().getPer_id()%>&id='+job_id,function(resData){
					ajax_remove();
					if (resData.status == 'success') {
			...
			<div class="content_body">
				<div class="left">
					<form style="margin: 0; padding: 0;" action="sv_job_manage.jsp" id="search" method="get">
						...
						&nbsp;&nbsp;
						JOB ID: <input type="text" name="id" class="txt_box" value="<%=id%>"> 
						&nbsp;&nbsp;
						Status: 
					   ...
					   <tr> 
						 <td colspan="10" style="padding: 0px 0px 0px 0px;" width="100%">
							...
							<tr> 
								...
								<td align="left" width="11%"><%=entity.getForewordname()+"	"+entity.getCus_name()%></td>
									...
									if(entity.getService_type().equals(ServiceSale.SERVICE_MA)){
										...
										<% } else if(entity.getStatus().equalsIgnoreCase(ServiceSale.STATUS_CLOSED)){ %>
											...
											<% 												    
											 ...
											 if(CheckRoleCount > 0){
											%>
												<button class="btn_reload"  onclick="Restatus(<%=entity.getId()%>);"  title="Restatus"></button>
										    <%}%>
										    
										    <%
										    if(!search.equalsIgnoreCase("") && !repair.getRepair_type().equalsIgnoreCase("12")){	
										    %>										    	
										   		<a class="btn_forward" href="sv_job_info.jsp?id=<%=entity.getId()%>&status=forward" title="Forward Job"></a>
										    <%}%>
