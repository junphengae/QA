	...
	$('#btn_approve').click(function(){
		if (confirm('ยืนยันการอนุมัติใบสั่งซื้อเลขที่ : '+pono)) {
			var update_by = '<%=securProfile.getPersonal().getPer_id()%>';
				...
				ajax_load();
				$.post('../PurchaseManageServlet',{'action':'approve_po','po':pono,'update_by':update_by},function(resData){
					ajax_remove();
	...
	$('#btn_reject').click(function(){
		if (confirm('ยืนยันการยกเลิกใบสั่งซื้อเลขที่ : '+pono)) {
			var update_by = '<%=securProfile.getPersonal().getPer_id()%>';
				...
				ajax_load();
				$.post('../PurchaseManageServlet',{'action':'reject_po','po':pono,'update_by':update_by},function(resData){
					ajax_remove();
					if (resData.status == 'success') {
				...
				<div class="s_auto right ">
					<table>
						...
						<tr>
							...
							<td align="right">
								<span id="span_gross_amount"><%=Money.money(PO.getGross_amount()) %></span>
						...
						<tr>
							...
							<td align="right">
								<strong>ส่วนลด (Discount)</strong>
								 <input type="hidden" name="discount_pc" id="discount_pc" value="<%=PO.getDiscount_pc()%>"><%=PO.getDiscount_pc()%> %
							</td>
							<td align="right"><span id="span_discount">
							<%String dis_sum = Money.subtract(PO.getGross_amount() , Money.discount(PO.getGross_amount() , PO.getDiscount_pc()));%>
								<%=Money.money(dis_sum) %>
							</span></td>
						</tr>
						<tr>
						...
						<tr>
							...
							<input type="hidden" name="vat_amount" id="vat_amount" value="0"></td>
						</tr>
						<tr class="txt_bold">
							<td colspan="2"></td>
							...
							<td align="right">
								...
								<input type="hidden" name="grand_total" id="grand_total" value="0"></td>
						</tr>
					</table>
				</div>
				<div class="clear"></div>
					...
					<table class="bg-image s_auto">
						<thead>
							<tr>
								...
								<th valign="top" align="center" width="13%">หน่วยนับ<br>(Units)</th>								
								<th valign="top" align="center" width="13%">จำนวน<br>(Quantity)</th>
						...
						<tbody>
							<%
								...
								<tr id="tr_<%=entity.getId()%>" valign="middle">
									...
									<td align="center"><%=Money.moneyInteger(entity.getOrder_qty())%></td>
									<td align="center"><%=Money.money(entity.getOrder_price())%></td>
