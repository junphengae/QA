					...
					<table width="100%">
						<tbody>
							<tr>
								...
								<td width="60%">: <%=invMaster.getUISubCat().getUICat().getUIGroup()
					.getGroup_name_th()%></td>
							</tr>
							<tr>
								<td>ชนิด</td>
							...
							<tr>
								<td>รูปแบบสินค้า</td>
								<td>: <%=(invMaster.getSerial().equalsIgnoreCase("y") ? "มี serial"
					: "ไม่มี serial")%></td>
							</tr>
							<tr>
								<td>รหัสสินค้า</td>
							...
							<tr>
								...
								<td>: <%=(invMaster.getFifo_flag().equalsIgnoreCase("y")) ? "FIFO"
					: "Non FIFO"%></td>
							</tr>
							<tr>
							...
							<table width="100%">
								<tbody>
									...
									<tr>
										...
										<td width="70%">: <%=WebUtils.getReqString(request, "auto_id")%></td>
									</tr>
									<tr id="tr_lot_no">
									...
									<tr>
										<td colspan="2" align="center">
											<input type="hidden" name="auto_id" value="<%=WebUtils.getReqString(request,"auto_id")%>">	
											<input type="hidden" name="request_no" value="<%=WebUtils.getReqString(request,"auto_id")%>">	
											...
											<input type="hidden" name="action" value="outlet_service_serial"> 
											<input type="hidden" name="mat_code" value="<%=mat_code%>">
											<input type="hidden" name="request_type" value="SV"> 
									...
									if($('#request_type').val() == ''){
										...
										$.post('../OutletManagement',data,function(resData){
											...
											if (resData.status == 'success') {
												...
												window.location = 'inv_borrow.jsp';
											} else {
				...
				<fieldset class="s900 fset">
					...
					<table class="bg-image s900">
						...
						<%
							String up = "0";
							Double total = 0.0;
							Iterator iteLot = InventoryLot.selectActiveList(mat_code)
									.iterator();
