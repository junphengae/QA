...
vehicle = Vehicle.select(vid);
VehicleMaster vMaster = VehicleMaster.select(vehicle.getMaster_id());
%>
<%@ taglib uri="/WEB-INF/lib/customtag.tld" prefix="snc" %>
	...
	<div class="wrap_body">
		<div class="body_content">
			<div class="content_head">
				<div class="left">Edit Repaired List : เลขที่ใบแจ้งซ่อม: <%=repair_order.getId()%>  | <a class="thickbox btn_box" lang="../info/vehicle_info.jsp?width=400&height=250&vid=<%=vid%>" title="ข้อมูลรถ">ทะเบียนรถ: <%=vehicle.getLicense_plate()%></a> | <a class="thickbox btn_box" lang="../info/customer_info.jsp?width=400&height=250&cus_id=<%=cus_id%>" title="ข้อมูลลูกค้า">ข้อมูลลูกค้า: คุณ<%=customer.getCus_name_th()%></a></div>
				<div class="right">
		...
		<div class="content">
			...
			<div class="box_wrap ">
				...
				<div class="box_body">
					<fieldset class="fset s910 center" id="customer_info">
					...
					<fieldset class="fset s910" style="display: none;" >
			...
			<fieldset class="fset s910" >
				...
				<div class="box_body">
					<table class="bg-image s910 center">
						<thead>
							<tr>
								<th width="80px" align="center">รหัส</th>
						...
						<%
							...
							while (iteLabor.hasNext()) {
								...
								String onRemove = "javascript: if(confirmRemove(this)){removeRepairList(this),deleteService('"+id+"','"+labor.getNumber()+"','"+laborTime.getLabor_en()+"/"+laborTime.getLabor_th()+"');}";								
								...
								while (mechanicIte.hasNext()){
									...
									if (mechanic.getStatus().equalsIgnoreCase(RepairLaborTime.STATUS_HOLDPART)) { // Status Hold Part
										mec_name += "<font style='color:red;'>" + Personal.selectOnlyPerson(mechanic.getMechanic_id()).getName() + " [HOLD]</font>,";
									} else {
						...
						%>
							<tr id="list_<%=labor.getLabor_id() %>">
								...
								%>
									QC ผ่าน
								<%
									} else if (status.equalsIgnoreCase(RepairLaborTime.STATUS_CLOSED)) {
								%>
									จบงาน
								<%
									} else {
								%>
									<div class="btn_del2" number="<%=labor.getNumber()%>" id="list_<%=labor.getLabor_id() %>" title="<%=removeMsg%>" onclick="<%=onRemove%>" lang="<%=labor.getLabor_id() %>"></div>
								<%
									}
								%>
								</td>
				...
				<div class="txt_center" style="margin: 5px 0 5px 0;">
					<input type="button" name="next" class="btn_box" value="เลือกช่าง" onclick="javascript: window.location='sa_job_active_repaired_assign_mechanic.jsp?id=<%=id%>&vid=<%=vid%>&cus_id=<%=cus_id%>';">
