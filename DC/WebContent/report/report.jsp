...
$(function(){
	...
	if(document.getElementById("report_type").value != "1" ){
				...
				}else if (document.getElementById("report_type").value == "part_stock_branch") {
					
					if (document.getElementById("type_date").checked || document.getElementById("type_month").checked ||  document.getElementById("type_date_date").checked == false) {
						alert("คุณยังไม่ได้ระบุ ช่วงเวลา ที่ต้องการดูรายงาน");
					}
				}				
				else				
				{
	...
	$('#report_type').change(function(){
		...
		if(type == "1") {
			...
			tr_month.hide();
			tr_date_date.hide();
		}else if( type == "part_stock_branch" ){ //รายงานสรุปการขายรวม All 
			tr_matcode.hide();
			tr_branch.show();
			tr_type_time.show();
			tr_date.hide();
					...
					<tr>
						...
						<td>: 
							<bmp:ComboBox name="report_type" styleClass="txt_box s320" value="">
								...
								<bmp:option value="<%=ReportUtils.PART_STOCK_BRANCH%>" text="รายงานสรุปการขายรวม(All)"></bmp:option> 								
							</bmp:ComboBox>
						</td>
					</tr>
