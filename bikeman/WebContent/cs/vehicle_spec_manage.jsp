...
function selectVehicle(id,cus_id,create_by){
	if (confirm('Selected!')) {
		ajax_load();
		$.post('../VehicleManage',{'action':'select_vehicle','master_id':id,'cus_id':cus_id,'create_by':create_by},function(resData){
			//tb_remove();
			if(resData.status == 'success'){
				//window.location.reload();
				window.location = "customer_info.jsp?cus_id=<%=cus_id%>";
			}else {
	...
	<div class="wrap_body">
		<div class="body_content">
			...
			<div class="content_body">
				...
				<div class="right txt_center"><%=PageControl.navigator_en(ctrl,"vehicle_spec_manage.jsp", paramList)%></div>
				<div class="clear"></div>
					...
					%>
						<tr>
							...
							<td align="center">
								...
								<a class="btn_accept" onclick="selectVehicle('<%=entity.getId()%>','<%=cus_id %>','<%=securProfile.getPersonal().getPer_id() %>')" title="Select"></a>
								<%-- href="customer_info.jsp?id=<%=entity.getId()%>&cus_id=<%=cus_id %>"  --%>
							</td>
						</tr>
					<%
						}
