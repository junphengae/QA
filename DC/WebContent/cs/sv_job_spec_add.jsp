...
<title>Create Vehicle Spec</title>
<%
	...
	var v = $form.validate({
		submitHandler: function(){
			...
			$.post('../VehicleManage',addData,function(resData){
				ajax_remove();
				if (resData.status == 'success') {
					$msg.text('Success').show().delay(500).queue(function(){window.location = "sv_job_customer_info.jsp?cus_id=<%=cus_id%>";$(this).dequeue();});
				} else {
					alert(resData.message);
				}
			},'json');
					...
					<tbody>
						...
						<tr align="center" valign="bottom" height="30">
							<td colspan="2">
								...
								<input type="reset" onclick="window.location='sv_job_customer_info.jsp?cus_id=<%=cus_id%>';" value="Back" class="btn_box">
							</td>
						</tr>
					</tbody>
				</table>
