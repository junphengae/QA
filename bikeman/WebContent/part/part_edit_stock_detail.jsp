...
<%
String mat_code = WebUtils.getReqString(request, "pn");//*** Part Number ***//
PartMaster entity = PartMaster.select(mat_code);//****SELECT  pa_part_master****//

String po_no =   WebUtils.getReqString(request, "po");//**** PO Number *****//

PurchaseRequest entityPR = new PurchaseRequest();
WebUtils.bindReqToEntity(entityPR, request);
PurchaseRequest.select(entityPR);//****SELECT pur_purchase_request*****//

InventoryPacking Packing = InventoryPacking.selectmat_code(entity.getPn()); //****SELECT inv_packing *****//

//ทำ replace เนื่องจากถึงหลักพันแล้ว error ต้องเอา "," ออกเนื่องจากคำนวนเป็น int
String recive_qty_rep = Money.moneyInteger(entityPR.getUIrecive_qty()).replace("," ,""); //***จำนวน ที่ sum มาจาก  part_lot***//


//************************************************************************************/
String recive_qty = "0"; //** จำนวนทที่ได้รับ   **//
...
var packing = "<%=Packing.getUnit()%>";
 ...
 if(Description.val() != 0){  /*** start Description > 0 ******/
	...
	$('#pn_sn').focus();
	$('#pn_sn').bind("keypress" , function(e){
        if (e.which == 13) {
            if(running === false) {
                running = true;
                submitSN();	
	...
	/*******************  *********/
	$('#non_sn_qty').focus();	
 	$('#non_sn_qty').bind("keypress" , function(e){
        if (e.which == 13) {
            if(running === false) {
                running = true;
                submitNonSN(); 
 	...
 	function submitSN(){
 		var msg = $('#sn_msg_error');		
			if (pn_sn.val() == "") {
				...
				ajax_load();				
				$.post('../PartInventory',form.serialize()+'&pn_sn='+pn_sn.val().trim()+'&qty=1&pn='+pn.val().trim()+'&po='+po.val()+'&action=stock_in_sn_edit',function(resData){
				ajax_remove();				
 	...
 	function submitNonSN(){//*btn_save_non_sn*//		
		...
		if (non_sn.val() == "" || non_sn.val() == "0" || !(/^\d+$/.test(non_sn.val()))) {
			...
			if(add_qty>qty_hand){/****ถ้าจำนวนที่ Edit มากกว่า Order****/
				
				if (confirm('ท่านกำลังแก้ไขจำนวนอะไหล่เกินกว่าจำนวนที่จัดส่ง ต้องการทำต่อไปหรือไม่')) {
					ajax_load();				
					$.post('../PartInventory',form.serialize()+'&qty='+non_sn.val().trim()+'&pn='+pn.val().trim()+'&po='+po.val()+'&action=stock_in_non_sn_edit',function(resData){
					ajax_remove();				
						if (resData.status == 'success') {
							window.location.reload();
...
<!-- /************************************************************************************************************************/ -->
				  ...
				  <div id="div_non_sn" >		   					
							...
							<div class="left">		
									<input type="radio" name="normal" id="normal0" value="0"  class="radioclick" > 
									<input type="hidden" name="inv_type0" id="inv_type0" value="<%=UnitType.selectName(entity.getDes_unit())%>"  class="radioclick">
									<label for="upImg">ปกติ</label>
									...
									<%=UnitType.selectName(entity.getDes_unit())%>
							</div>	
							<br/>	
							<%} %>								
							<%
							...
							%>							
							<div class="left">
								<input type="radio" name="normal" id="normal<%=i%>" value="<%=i%>"  class="radioclick">
								<input type="hidden" name="inv_type<%=i%>" id="inv_type<%=i%>" value="<%=entityPacking.getDescription()%>" >
								<label for="upImg"><%=entityPacking.getDescription()%></label>									
								...
								<%=entityPacking.getDescription()%> (<%=entityPacking.getDescription()%>ละ <%=entityPacking.getUnit()%> <%=UnitType.selectName(entity.getDes_unit())%>) 
							</div>
							...
							<div id="show" class="hide">
										...
										<div>
											...
											<div class="s80 left">New Qty </div><div class="s10 left"> : </div>
											<div class="s180 left">
												<div class="left">
													<input type="text" name="non_sn_qty" id="non_sn_qty" class="txt_box s80" value=""> 
													&nbsp;
													<button type="submit" id="btn_save_non_sn" class="btn_box "> Edit stock</button>
												</div>
												<div class="clear"></div>
											</div><div class="clear"></div>
